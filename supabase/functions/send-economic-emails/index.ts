...
async function downloadFileAsBase64(supabaseClient: SupabaseClient, bucket: string, url: string): Promise<{ content: string, filename: string, mimeType: string }> {
  console.log(`[send-economic-emails] v3: Downloading file from URL: ${url} in bucket: ${bucket}`);
  ...
  console.log(`[send-economic-emails] v3: Extracted and decoded file path: ${filePath}`);
  ...
  if (downloadError) {
    console.error(`[send-economic-emails] v3: Supabase storage download error for path ${filePath}:`, downloadError);
  ...
  console.log(`[send-economic-emails] v3: Successfully downloaded and encoded file: ${filename}, size: ${buffer.byteLength} bytes`);
...
serve(async (req) => {
  ...
  console.log('[send-economic-emails] v3: Function invoked.');
  ...
  try {
    ...
    if (!resendApiKey || !supabaseUrl || !supabaseServiceRoleKey) {
      console.error('[send-economic-emails] v3: Missing environment variables.');
    ...
    console.log('[send-economic-emails] v3: Received payload:', JSON.stringify(payload, null, 2));
    ...
    if (!recipientEmail) {
      console.log(`[send-economic-emails] v3: No economic recipient configured for country: ${country}. Skipping email.`);
    ...
    if (table === 'payment_requests' && record.status === 'approved' && record.receipt_pdf_url) {
      ...
      console.log(`[send-economic-emails] v3: Processing Payment Request Approved: ${record.id}`);
      ...
      console.log(`[send-economic-emails] v3: Processing Transaction Completed: ${record.id}`);
    } else {
      console.log(`[send-economic-emails] v3: Record does not match required criteria. Table: ${table}, Status: ${record.status}. Skipping.`);
    ...
    if (attachmentUrls.length > 0) {
      const attachmentPromises = attachmentUrls.map(url => 
        downloadFileAsBase64(supabaseClient, bucket, url).catch(e => {
          console.error(`[send-economic-emails] v3: Failed to download attachment from ${url}: ${e.message}`);
    ...
    if (attachments.length === 0) {
      ...
      console.warn(`[send-economic-emails] v3: No attachments processed for email with subject: ${subject}`);
    }

    console.log(`[send-economic-emails] v3: Sending email to ${recipientEmail} with subject: ${subject} and ${attachments.length} attachments.`);
    ...
    if (resendError) {
      console.error('[send-economic-emails] v3: Resend API Error:', resendError);
    ...
    console.log('[send-economic-emails] v3: Email sent successfully:', resendData);
    ...
    console.error('[send-economic-emails] v3: Unhandled Edge Function error:', error);
