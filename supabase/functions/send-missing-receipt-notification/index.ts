...
serve(async (req) => {
  ...
  try {
    ...
    const recipientEmail = '868bilag1677646@e-conomic.dk';

    const subject = `Missing Receipt - ${newRecord.entry}`;
    const htmlContent = `
      <p>Hello,</p>
      <p>A receipt has been added for transaction <strong>#${newRecord.id.substring(0, 8)}</strong> (Entry: ${newRecord.entry}).</p>
      ...
      for (const receiptUrl of newRecord.receipt_urls) {
        try {
          ...
          if (!receiptResponse.ok) {
            console.warn(`Failed to fetch receipt PDF from ${receiptUrl}: ${receiptResponse.statusText}. Skipping this attachment.`);
            continue;
          }
          ...
          console.error(`Error processing receipt URL ${receiptUrl}: ${fetchError.message}`);
        }
      }
    }

    if (attachments.length === 0) {
      console.warn('No valid receipt PDFs could be attached. Sending email without attachments.');
    }
    ...
    if (resendError) {
      console.error('Error sending missing receipt email via Resend:', resendError);
    ...
    console.log('Missing receipt email sent successfully via Resend:', data);
