    ...
    queryFn: async () => {
      ...
      try {
        ...
        if (!accountingYears || accountingYears.length === 0) {
          showError("No accounting years found in e-conomic.");
          return [];
        }
        ...
        const fetchPromises = accountingYears.map(async (yearInfo: any) => {
          ...
          if (error) { console.error(`Error fetching entries for year ${yearInfo.year}:`, error.message); return []; }
          const resp = data as EconomicProxyResponse<any>;
          if (resp.error || !resp.ok) { console.error(`e-conomic error for year ${yearInfo.year}:`, resp.error || `Status ${resp.status}`); return []; }
