...
const UpdateStandingOrderForm: React.FC<UpdateStandingOrderFormProps> = ({ standingOrder, onStandingOrderUpdated }) => {
  ...
  const form = useForm<z.infer<typeof formSchema>>({
    ...
    defaultValues: {
      ...
      sku: standingOrder.sku || '',
      not_property_related: standingOrder.not_property_related || false,
      categories: formattedCategories,
      total_amount: standingOrder.total_amount,
      payment_reference: standingOrder.payment_reference || '',
      currency: standingOrder.currency || 'GBP',
    ...
    try {
      ...
      const { error } = await supabase
        ...
        .update({
          ...
          iban_number: standingOrder.country !== 'United Kingdom' ? values.iban_number : null,
          sort_code: standingOrder.country === 'United Kingdom' ? values.sort_code : null,
      ...
      dismissToast(toastId);
      showError(error.message || "Failed to update standing order.");
    }
  };

  const formCountry = standingOrder.country;
